...
void hasta_ekle() {
    ...
    // T.C. kimlik numarasý doðrulama
    do {
        printf("T.C. Kimlik Numarasini girin (11 haneli): ");
        scanf("%s", h.tc_kimlik_no);
        if (strlen(h.tc_kimlik_no) != 11) {
            printf("Hata: T.C. Kimlik Numarasi 11 haneli olmalidir!\n");
        }
    } while (strlen(h.tc_kimlik_no) != 11);
...
// Hastalari aciliyet seviyesine gore sirala 
...
void hasta_listele() {
    ...
    for (int i = 0; i < hasta_sayisi; i++) {
        printf("ID: %d | T.C. No: %s | Ad: %s | Yas: %d | Aciliyet: %s | Doktor: %s | Doktor ID: %d | Tani: %s | Recete No: %d\n",
            hastalar[i].id, hastalar[i].tc_kimlik_no, hastalar[i].ad, hastalar[i].yas, aciliyet_str(hastalar[i].seviye),
